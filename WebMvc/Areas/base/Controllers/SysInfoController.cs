...
namespace WebMvc
{
    ...
    {
        private readonly SysInfoApp _app;
        
        public SysInfoController(IAuth authUtil, SysInfoApp app) : base(authUtil)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Load(PageReq pageRequest, SysInfo entity)
        {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Ins(SysInfo Table_entity)
        {
            try
            {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Upd(SysInfo Table_entity)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Export(SysInfo entity)
        {
        ...
        public string GetTemplate()
        {
            ...
            List<SysInfo> listSysInfo = new List<SysInfo>();
            SysInfo entity = _app.FindSingle(u => u.Id > 0);
            if (entity != null)
            {
                listSysInfo.Add(entity);
            }
            else
            {
                listSysInfo.Add(new SysInfo());
            }
 
            result.data = listSysInfo;
            result.count = listSysInfo.Count;
