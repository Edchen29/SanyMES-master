...
namespace WebMvc
{
    ...
    {
        private readonly RepairApp _app;
        
        public RepairController(IAuth authUtil, RepairApp app) : base(authUtil)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Load(PageReq pageRequest, Repair entity)
        {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Ins(Repair Table_entity)
        {
            try
            {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Upd(Repair Table_entity)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Export(Repair entity)
        {
        ...
        public string GetTemplate()
        {
            ...
            List<Repair> listRepair = new List<Repair>();
            Repair entity = _app.FindSingle(u => u.Id > 0);
            if (entity != null)
            {
                listRepair.Add(entity);
            }
            else
            {
            ...
            result.data = listRepair;
            result.count = listRepair.Count;
