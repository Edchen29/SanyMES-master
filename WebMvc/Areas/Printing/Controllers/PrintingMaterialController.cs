...
namespace WebMvc
{
	...
	/// </summary>
    [Area("Printing")]
    public class PrintingMaterialController : BaseController
    {
        private readonly MaterialApp _app;
        
        public PrintingMaterialController(IAuth authUtil, MaterialApp app) : base(authUtil)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Load(PageReq pageRequest, Material entity)
        {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Ins(Material Table_entity)
        {
            try
            {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Upd(Material Table_entity)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Export(Material entity)
        {
        ...
        public string GetTemplate()
        {
            ...
            Material entity = _app.FindSingle(u => u.Id > 0);
            if (entity != null)
            {
                listMaterial.Add(entity);
            }
            else
            {
            ...
            result.data = listMaterial;
            result.count = listMaterial.Count;
