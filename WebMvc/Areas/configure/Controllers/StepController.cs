...
namespace WebMvc
{
    ...
    {
        private readonly StepApp _app;
        
        public StepController(IAuth authUtil, StepApp app) : base(authUtil)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Load(PageReq pageRequest, Step entity)
        {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Ins(Step Table_entity)
        {
            try
            {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Upd(Step Table_entity)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Export(Step entity)
        {
        ...
        public string GetTemplate()
        {
            ...
            Step entity = _app.FindSingle(u => u.Id > 0);
            if (entity != null)
            {
                listStep.Add(entity);
            }
            ...
            {
                listStep.Add(new Step());
            }
 
            result.data = listStep;
            result.count = listStep.Count;
        ...
        #region 自定义方法
        [HttpPost]
        public string UploadWorkSOP(IFormFile file,string pcode,string scode)
        {
            try
            {
                Response result = _app.UploadIForm(file, pcode, scode);
                if (!result.Status)
                {
                    Result.Status = false;
                    Result.Message = result.Message;
