...
namespace WebMvc
{
    ...
    {
        private readonly WorkshopApp _app;
        
        public WorkshopController(IAuth authUtil, WorkshopApp app) : base(authUtil)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Load(PageReq pageRequest, Workshop entity)
        {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Ins(Workshop Table_entity)
        {
            try
            {
        ...
        [HttpPost]
        [ServiceFilter(typeof(OperLogFilter))]
        public string Upd(Workshop Table_entity)
        {
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public string Export(Workshop entity)
        {
        ...
        public string GetTemplate()
        {
            ...
            List<Workshop> listWorkshop = new List<Workshop>();
            Workshop entity = _app.FindSingle(u => u.Id > 0);
            if (entity != null)
            {
                listWorkshop.Add(entity);
            }
            else
            {
                listWorkshop.Add(new Workshop());
            }
 
            result.data = listWorkshop;
            result.count = listWorkshop.Count;
